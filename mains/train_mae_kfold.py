...
from data.msseg_loader import msseg_k_fold, msseg_k_fold_test
...
matplotlib.use('Agg')
import cv2
from utils.utilization import mkdirs
import shutil
...
if __name__ == '__main__':
    ...
    shutil.copy('../scripts/train_recon.sh',
                os.path.join(summarypath, 'train_recon.sh'))  # copy config file to output folder
    ...
    if dataset == 'msseg':
        ...
        test_dataset = msseg_k_fold(folder, src=src_scan, trg=trg_scan, is_train=False, crop_size=opt.crop_size,
                                   resize=None, mni=True, fold=fold_k, normalize='self')
        ...
        with open(os.path.join(summarypath, 'val_%s_fold_%d.txt' % (src_scan['prefix'], fold_k)), 'w') as f:
            f.writelines("%s\n" % s for s in train_dataset.src_sub_list)
        res_block = 2
        evaluate_domain = evaluate_domain_2d
        activate = nn.Sigmoid()

    else:
        raise NotImplementedError

    #train_dataset = SliceADNI(TRAINING_DATA_PATH, TESTING_DATA_PATH, opt.crop_size, paired=False, normalize='global_max',
    #                          slice_num=opt.thickness, match=True, multi_axis=True, dim=opt.dim, cx=30, cy=70, cz=70, seg=True)
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for iteration, data in enumerate(data_loader_train):
            ...
            loss = class_loss + recon_loss #+ feat_loss
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
