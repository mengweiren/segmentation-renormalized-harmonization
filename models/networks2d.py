...
def define_G(input_nc, output_nc, ngf=64, netG='resunet', norm='batch', use_dropout=False, init_type='normal',
    ...
    if netG == 'unet_up':
        ...
        if spade:
            #net = ResSpadeUnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
...
class SharedResSpadeUnetGenerator(nn.Module):
    ...
    def forward(self, input, seg, freeze=False):
        #self.set_required_grad(not freeze)
        if freeze:
            self.shared_seg_feature.eval()
        else:
            self.shared_seg_feature.train()
        seg_feat, seg_rep, seg_off, loss = self.shared_seg_feature(input, seg)
        x = self.head(input)  # n, w/2, h/2
        x = F.relu(x)
        x_0 = self.down_spade_0(x, seg_feat, freeze)  # 2n, w/2, h/2
        x_1 = self.down_spade_1(self.down_1(x_0), seg_feat, freeze)  # 4n, w/4, h/4
        x_2 = self.down_spade_2(self.down_2(x_1), seg_feat, freeze)  # 8n, w/8, h/8
        x_3 = self.down_spade_3(self.down_3(x_2), seg_feat, freeze)  # 8n, w/16, h/16
        out = self.up(x_3) # 8n, w/8, h/8
        out = self.middle_0(out, seg_feat, freeze)  #8n, w/8, h/8
        out = self.middle_1(out, seg_feat, freeze)  #8n, w/8, h/8
        #out = self.up(out)  # 8n, w/8, h/8
        out = self.up_2(torch.cat([out, x_2], 1), seg_feat, freeze)  #4n, w/8, w/8
        out = self.up(out) #4n, w/4, w/4
        out = self.up_1(torch.cat([out, x_1], 1), seg_feat, freeze)  #2n, w/4, w/4

        out = self.up(out)   #2n, w/2, h/2
        out = self.up_0(torch.cat([out, x_0], 1), seg_feat, freeze)  #n, w/2, w/2

        out = self.up(out)   #n, w, h
        out = self.tail(F.leaky_relu(out, 2e-1))

        out = self.activate(out)
        return out, seg_rep, seg_off, loss
...
class SharedSPADEResBlock(SPADEResBlock):
    ...
    def forward(self, x, seg, freeze):
        x_s = self.shortcut(x, seg, freeze)

        dx = self.conv_0(self.actvn(self.norm_0(x, seg, freeze)))
        dx = self.conv_1(self.actvn(self.norm_1(dx, seg, freeze)))

        out = x_s + dx

        return out

    def shortcut(self, x, seg, freeze):
        if self.learned_shortcut:
            x_s = self.conv_s(self.norm_s(x, seg, freeze))
        else:
            x_s = x
        return x_s
...
class MultitaskAutoEncoder(nn.Module):
    def __init__(self, input_nc, numclass, ecfgs=[64, 'M', 128, 'M', 256, 256, 'M', 512, 512],
                 dcfgs=['U', 512, 512, 'U', 256, 256, 'U', 128, 64]):
        super(MultitaskAutoEncoder, self).__init__()
        self.input_nc = input_nc
        # self.feature = self.make_layers(in_channels=input_nc, cfg=ecfgs, norm=True)
        # self.reconstruct = self.make_layers(in_channels=512, cfg=dcfgs, norm=True)
        self.conv_0 = nn.Sequential(nn.Conv2d(self.input_nc, 64, kernel_size=3, stride=2, padding=1, bias=True),
                                    nn.InstanceNorm2d(64),
                                    nn.LeakyReLU(0.2))
        # nn.MaxPool3d(2,2))
        self.conv_1 = nn.Sequential(nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1, bias=True),
                                    nn.InstanceNorm2d(128),
                                    nn.LeakyReLU(0.2))
        # nn.MaxPool3d(2,2))
        self.conv_2 = nn.Sequential(nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1, bias=True),
                                    nn.InstanceNorm2d(256),
                                    nn.LeakyReLU(0.2))
        # nn.MaxPool3d(2,2))
        self.conv_3 = nn.Sequential(nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=True),
                                    nn.InstanceNorm2d(256),
                                    nn.LeakyReLU(0.2)
                                    )
        ...
        #self.classifier = nn.Linear(256 * 2 * 2 , numclass)

        self.up = nn.Upsample(scale_factor=2)
        self.up_3 = SharedSPADEResBlock(256, 128, 'spadeinstance3x3', 4)
        self.up_2 = SharedSPADEResBlock(128, 64, 'spadeinstance3x3', 4)
        self.up_1 = SharedSPADEResBlock(128, 64, 'spadeinstance3x3', 4)
        # self.up_0 = SPADEResBlock3d(64, 64, 'spadebatch3x3',4)
        self.final_conv = nn.Conv2d(64, input_nc, kernel_size=3, padding=1)

        self.shared_seg_feature = SegFeature(4)
        self.feat_criterion = nn.L1Loss(reduction='mean')
        #self.init_weight()
    ...
    def forward(self, x, seg):
        ...
        x3 = self.conv_3(x2)
        # print(seg.size())
        #print('x0, x1, x2, x3\n', x0.size(), x1.size(), x2.size(), x3.size())

        feat = self.avgpool(x3)
        #print(feat.size())
        pred = torch.flatten(feat, 1)
        pred = self.classifier(pred)
        if seg is None:
            # test time: does not do reconstruction
            return pred, feat

        seg_feat, _, _, _ = self.shared_seg_feature(x, seg)

        r3 = self.up(x3)
        r3 = self.up_3(r3, seg_feat, False)
        r2 = self.up(r3)
        r2 = self.up_2(r2, seg_feat, False)
        r1 = self.up(r2)
        #r1 = self.up_1(r1, seg_feat, False)
        rec = self.final_conv(r1)
        # rec = self.up_0(r1, seg_feat)
        #print('r0, r1, r2, r3\n',  rec.size(),  r1.size(), r2.size(), r3.size())

        # print(rec.size())

        # print(rec.size())
        feat_loss = self.feat_criterion(x0, r2) + self.feat_criterion(x1, r3) #+ self.feat_criterion(x2, r3)
        # print(pred.size())
        return pred, feat, rec, feat_loss

    def make_layers(self, in_channels, cfg, norm=False):
        layers = []
        for v in cfg:
            if v == 'M':
                ...
                conv3d = nn.Conv2d(in_channels, v, kernel_size=3, padding=1)
                if norm:
                    layers += [conv3d, nn.BatchNorm2d(v), nn.LeakyReLU(0.2, True)]
                else:
                    layers += [conv3d, nn.LeakyReLU(0.2, True)]
                in_channels = v
